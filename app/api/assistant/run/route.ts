...
import { createMessage, createThread, getAssistant, getThread, retrieveMessagesFromThread, retrieveRunFromThread, runAssistant, updateAssistant } from "@/app/utils/OpenAI";
...
export async function POST(req: Request) {
  try {
    ...
    const thread = threadId && typeof threadId === 'string' ? await getThread(threadId) : await createThread()
    ...
    while(run.status !== 'completed'){
      if(run.status === 'cancelled' || run.status === 'failed' || run.status === 'expired') {
    ...
    const resp = new Response(JSON.stringify({messages, threadId: thread.id}), {
      status: 200
    })
    return resp
  } catch (error) {
